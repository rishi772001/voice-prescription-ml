...
def verify(name=None):
	...
	if name:
		p = "F:/Mini Projects Rishi/voice prescription/main/{}.docx".format(name)
...
def doc():
	...
	try:
		...
		# Extract text
		name, age, date, medicines = extract_details(text)
		# Create document
		create(name, age, date, medicines)
		# Verify document
		verify(name)
	except:
...
def listen():
	...
	try:
		...
		messagebox.showinfo("Alert", "Converted Text:{}".format(text))
		# Extract text
		name, age, date, medicines = extract(text)
		# Create document
		create(name, age, date, medicines)
		# Verify document
		verify(name)
